    ...
    public static class Render
    {
        ...
        public static void RenderInteractables()
        {
            if (TeleporterInteraction.instance)
            {
                ...
                {
                    View.Styles.RenderTeleporterStyle.normal.textColor =
                    ...
                    GUI.Label(new Rect(BoundingVector.x - 50f, (float)Screen.height - BoundingVector.y, 100f, 50f), boxText, View.Styles.RenderTeleporterStyle);
                }
            }
            ...
            for (int i = 0; i < barrelInteractions.Count; i++)
            {
                ...
                if (!barrel.Networkopened)
                {
                    ...
                    if (BoundingVector.z > 0.01)
                    {
                        ...
                        string boxText = $"{friendlyName}\n{distance}m";
                        GUI.Label(new Rect(BoundingVector.x - 50f, (float)Screen.height - BoundingVector.y, 100f, 50f), boxText, View.Styles.RenderInteractablesStyle);
                    }
                }
            }
            ...
            for (int i = 0; i < secretButtons.Count; i++)
            {
                ...
                if (secretButton)
                {
                    ...
                    if (BoundingVector.z > 0.01)
                    {
                        ...
                        string boxText = $"{friendlyName}\n{distance}m";
                        GUI.Label(new Rect(BoundingVector.x - 50f, (float)Screen.height - BoundingVector.y, 100f, 50f), boxText, View.Styles.RenderSecretsStyle);
                    }
                }
            }
            ...
            for (int i = 0; i < hurtBoxes.Count; i++)
            {
                ...
                if (mob)
                {
                    ...
                    if (MobBoundingVector.z > 0.01)
                    {
                        ...
                        GUI.Label(new Rect(MobBoundingVector.x - 50f, (float)Screen.height - MobBoundingVector.y + 50f, 100f, 50f), mobBoxText, View.Styles.RenderMobsStyle);
        ...
        public static void RenderActiveMods()
        {
            ...
            Dictionary<string, bool> allMods = new Dictionary<string, bool>()
            {
                ...
                { "Keyboard-Navigation", View.UmbraModGUI.Instance.navigationToggle },
            ...
            if (modsActive != null)
            {
                ...
                if (modsBoxText != "")
                {
                    GUI.Label(new Rect(Screen.width / 16, bottom.y - 55f, 200, 50f), "Active Mods: ", View.Styles.ActiveModsStyle);
                    GUI.Label(new Rect((Screen.width / 16) + 124, bottom.y - 55f, Screen.width - (Screen.width / 6), 50f), modsBoxText, View.Styles.ActiveModsStyle);
                }
            }
        }
        ...
        {
            var chest = Chests.FindClosestChest();
            ...
            if (chestBoundingVector.z > 0.01)
            {
                ...
                {
                    GUI.Label(new Rect(chestBoundingVector.x - 50f, (float)Screen.height - chestBoundingVector.y + 35f, 100f, 50f), $"Selected Chest", View.Styles.SelectedChestStyle);
                }
                else
                {
                    GUI.Label(new Rect(chestBoundingVector.x - 50f, (float)Screen.height - chestBoundingVector.y + 35f, 100f, 50f), $"Selected Chest\n{dropNameColored}", View.Styles.SelectedChestStyle);
                }
